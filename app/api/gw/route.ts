...
function json(data:any, init:any = {}) { return new NextResponse(JSON.stringify(data), { status: 200, headers: { "content-type": "application/json", ...init.headers } }); }
const clamp = (x:number, a:number, b:number)=>Math.max(a, Math.min(b, x));

/** --- Dexscreener + GMGN ingest (lightweight) --- **/
async function fetchUniverse(): Promise<TokenSignal[]> {
  // Dexscreener: trending + recently added
  // Using two endpoints and merging. (Edge-safe simple fetch)
  const urls = [
    "https://api.dexscreener.com/latest/dex/tokens/solana",
    "https://api.dexscreener.com/latest/dex/search?q=solana"
  ];
  const resps = await Promise.allSettled(urls.map(u=>fetch(u, { cf:{ cacheTtl: 10, cacheEverything: true }})));
  ...
  for (const r of resps) {
    ...
    for (const p of pairs.slice(0, 200)) {
      ...
      out.push({
        ...
        routeHealth: 1 // quick default; refined by quote checks if needed
      });
    }
  }
  // Deduplicate by mint
...
/** --- Scoring + bundle compose (fresh/updated focus) --- **/
...
function composeBundles(universe:TokenSignal[]) {
  ...
  function mk(name:string, kind:string, items:TokenSignal[], n:number) {
    ...
    return {
      ...
      createdAt: now,
      metrics: {
        w1h: toks.reduce((s,t,i)=> s + (t.r1h||0), 0)/Math.max(1,toks.length)
      },
  ...
  // 20% majors/memes (anchor)
  bundles.push(mk("Majors & Memes","majors", majors, Math.min(6, majors.length||3)));

  // 20% strategies
  bundles.push(mk("Index Mix","index", [ // SOL + BURRITO + top flows
    {address: MINT_SOL, symbol:"SOL", price:0, r1m:0,r5m:0,r1h:0,vol5m:0,vol1h:0,vol24h:0,liqUsd:0,trades24h:0,routeHealth:1},
    {address: process.env.BURRITO_MINT!, symbol:"BURRITO", price:0,r1m:0,r5m:0,r1h:0,vol5m:0,vol1h:0,vol24h:0,liqUsd:0,trades24h:0,routeHealth:1},
    ...rest.slice(0,4)
  ], 6));
...
async function jupQuote(inputMint:string, outputMint:string, amount:number, slippageBps=100) {
  // amount in smallest units for SOL (lamports) or token decimals; for demo we assume SOL in lamports
...
export async function POST(req:NextRequest) {
  ...
  if (op === "quote") {
    ...
    // Prepare quotes: 1) BURRITO 1%  2) each token leg
    const lamports = Math.round((amountSol * 0.99) * 1e9); // just used for proportional display
    const routes:any[] = [];
    // NOTE: For a real swap, call Jup /swap to build the transaction. Here we just show we can quote.
    // Example: quote SOL->each token with proportional lamports.
    for (const leg of alloc?.perToken ?? []) {
      const amt = Math.max(10000, Math.round(lamports * leg.weight)); // >= 0.00001 SOL
